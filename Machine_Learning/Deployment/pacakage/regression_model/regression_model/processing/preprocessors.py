...
class CategoricalImputer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series = None
            ...
            """ Fit statement to accomodata the sklearn pipeline."""
...
class RareLabelCategoricalEncoder(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, tol=0.05, variable=None):
        self.tol = tol
        if not isinstance(variables, list):
            self.variables = [variables]
        else:
    ...
    def fit(self, X, y=None):
        #persit frequent labels in dictionary
        self.encoder_dict_ = {}
    ...
    def transform(self, X):
        X = X.copy()
        for feature in self.variables:
            X[feature] = np.where(X[features].isin(
...
class CategoricalEncoder(BaseEstimator, TransformerMixin):
    """String to numbers catgorical encoder."""
    ...
    def transform(self, X):
        ...
        if X[self.variables].isnull().any().any():
            ...
            vars = {key: value for (key, value) in null_counts.items()
                    if value is True}
...
class DropUnecessaryFeatures(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, variable_to_drop=None):
        self.variables = variable_to_drop
