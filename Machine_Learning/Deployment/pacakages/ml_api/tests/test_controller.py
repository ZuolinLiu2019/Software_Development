...
from neural_network_model.config import config as cnn_config
...
def test_prediction_endpoint_returns_prediction(flask_test_client):
    ...
    post_json = test_data[0:1].to_json(orient='record')
...
def test_classifier_endpoint_returns_prediction(flask_test_client):
    # Given
    # Load the test data from the neural network model package
    # This is import as it makes it harder for the test
    # data version to get confused by not spreading it across package

    data_dir = os.path.abspath(os.path.join(cnn_config.DATA_FOLDER, os.pardir))
    ...
    response = flask_test_client.post('/predict/classifier',
                                    content_type='multiple/form_data',
                                    data=data)
    # Then
    assert response.status_code == 200
    response_json = json.loads(response.data)
